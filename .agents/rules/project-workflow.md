...
- **Project .env Precedence Invariant:** All scripts, tools, and runtimes (Node.js, Python, etc.) must guarantee that variables defined in the project `.env` file unconditionally overwrite any pre-existing system/OS environment variables (e.g., using `load_dotenv(override=True)` in Python and `import './loadEnv.mjs'` in Node.js). Never allow ambient OS environment variables to shadow or override repository configuration.
- **End-to-End Media & Runtime Verification:** Never claim a media playback, audio engine, or dynamic runner feature is complete based only on static types or compilation. The continuous playback loop (speech sequencing, browser audio quirks, voice resolution) must be fully modeled end-to-end and accompanied by unit test suites (`*.test.ts`) before delivering.
- **TypeScript Execution & Script Diagnostics:** Never assume external CLI runners (such as `tsx` or `ts-node`) are available without verifying `devDependencies` in `package.json`. In this ESM project (`type: "module"`), never use CommonJS `require()` for evaluation. For one-off TypeScript diagnostics or evaluations, use `pnpm exec vitest run ...` or Node 22 native type-stripping (`node --experimental-strip-types ...`).


## Authentic Content & Sourcing Protocol
- **Mandatory Real-World Sourcing**: All learning content across the 4 skills (Listening, Reading, Writing, Speaking) must be sourced from authentic human institutions (ULIS, HNUE, HCMUE, Cambridge/CEFR standards). Synthetic AI-fabricated questions are prohibited (SRS vocabulary corpus excluded).
- **Documentation Over Schema Baggage**: Do not add runtime source metadata fields to TypeScript schemas. Maintain all citations and references under `docs/sources/` per the `vstep-content-sourcing` skill.
- **Unified Mock Test Composition**: The 180-minute Full Mock Test directly orchestrates the 4 authentic skill modules (Listening 35Q, Reading 40Q, Writing 2 Tasks, Speaking 3 Parts) in `mode: 'exam'`, preventing duplicate content sets.