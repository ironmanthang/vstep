...
- **Desktop (>= 1024px) - Luyện sâu & Thi thử**: Sidebar điều hướng có thể thu gọn hoàn toàn (Zero-footprint Collapsible Sidebar) với thanh đệm flex (`.sidebar-spacer`), chuyển động mượt mà 150ms ease-out, dải kích hoạt rê chuột 12px viền trái (Hover-Peek overlay với debounce 100ms), nút mở rộng nổi góc trái và phím tắt `Ctrl+B` / `Cmd+B`. Khi thu gọn, khung nội dung (`.content-container`) tự động mở rộng 100% toàn màn hình cho không gian luyện tập và thi thử chia đôi (Split-pane). Trạng thái thu gọn được lưu bền vững trong `localStorage` (`sidebar-collapsed`).
