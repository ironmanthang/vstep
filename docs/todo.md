...
- [x] fix status bar color & icon contrast in light mode: Chuẩn hóa `apple-mobile-web-app-status-bar-style` thành `default` trong Light Mode, đồng bộ động `theme-color` (`#F0ECE3` cho Light Mode, `#141210` cho Dark Mode) và `colorScheme`, xử lý triệt để lỗi thanh trạng thái đen nuốt mất biểu tượng pin/wifi/giờ trên Android/iOS.