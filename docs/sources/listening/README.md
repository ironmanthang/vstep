...
- **Modern Ingestion & Mining Pipeline**:
  ...
  - **Vocabulary Mining Engine (`scripts/mine_listening_vocab.mjs`)**: Mines high-yield VSTEP B1–C1 vocabulary items from all 22 listening test transcripts and questions. Enriches candidates via Gemini Flash model cascade (`gemini-3.8-flash` -> `3.7` -> `3.6` -> `3.5-flash-lite`) to expand the Master Flashcard Corpus (+500 cards, 1,500 -> 2,000 cards) with 100% authentic exam-context examples.