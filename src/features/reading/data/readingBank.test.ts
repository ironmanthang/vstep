...
describe('Authentic ULIS Reading Test 5 Integrity', () => {
  it('should have exactly 4 passages totaling 40 questions', () => {
    expect(ULIS_READING_TEST_05.passages.length).toBe(4);
    const totalQ = ULIS_READING_TEST_05.passages.reduce((acc, p) => acc + p.questions.length, 0);
    expect(totalQ).toBe(40);
  });

  it('each question should have 4 choices, valid key and non-empty explanation', () => {
    ULIS_READING_TEST_05.passages.forEach((p) => {
      expect(p.content_paragraphs.length).toBeGreaterThan(1);
      expect(p.questions.length).toBe(10);
      p.questions.forEach((q) => {
        expect(q.options.length).toBe(4);
  ...
  it('all 40 questions must match the official answer key from PDF page 149', () => {
    const allQuestions = ULIS_READING_TEST_05.passages.flatMap((p) => p.questions);
    allQuestions.forEach((q, idx) => {
      expect(q.correct_key).toBe(TEST5_OFFICIAL_KEYS[idx]);
    });
  });
...
describe('Authentic ULIS Reading Test 6 Integrity', () => {
  it('should have exactly 4 passages totaling 40 questions', () => {
    expect(ULIS_READING_TEST_06.passages.length).toBe(4);
    const totalQ = ULIS_READING_TEST_06.passages.reduce((acc, p) => acc + p.questions.length, 0);
    expect(totalQ).toBe(40);
  });

  it('each question should have 4 choices, valid key and non-empty explanation', () => {
    ULIS_READING_TEST_06.passages.forEach((p) => {
      expect(p.content_paragraphs.length).toBeGreaterThan(1);
      expect(p.questions.length).toBe(10);
      p.questions.forEach((q) => {
        expect(q.options.length).toBe(4);
  ...
  it('all 40 questions must match the official answer key from PDF page 154', () => {
    const allQuestions = ULIS_READING_TEST_06.passages.flatMap((p) => p.questions);
    allQuestions.forEach((q, idx) => {
      expect(q.correct_key).toBe(TEST6_OFFICIAL_KEYS[idx]);
    });
  });
